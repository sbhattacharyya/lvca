...
public class SymbolTree
{
    ...
    void addChild(SymbolTree childTree) {
        // Match the outer tree
        SymbolTree match = null;
        for (SymbolTree child : children) {
            if (child.name.equals(childTree.name)) {
                match = child;
                break;
            }
        }
        ...
        if (match == null) {
            ...
            // The outer tree matches
            // Try and add each of the children of the childTree
            for (SymbolTree proposedAddChild : childTree.children) {
                if (match.children.stream().noneMatch(c -> c.name.equals(proposedAddChild.name))) {
                    match.children.add(proposedAddChild);
                }
            }
        }
