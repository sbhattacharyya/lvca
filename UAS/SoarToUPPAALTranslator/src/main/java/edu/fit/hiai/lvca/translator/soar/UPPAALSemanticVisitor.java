...
public class UPPAALSemanticVisitor extends SoarBaseVisitor<Node> {
     ...
     * @return Whether to generate noneButSwitch
     */
    private boolean getGlobalDeclaration() {
        ...
        addConstantToBuilder(globalVariables, "dummy_io1", index++);
        addConstantToBuilder(globalVariables, "dummy_io2", index++);
        addConstantToBuilder(globalVariables, "dummy_io3", index++);
        ...
        //index is set to latest negative number.  Currently, -13. Based on what other values are needed , you can move this further negative. These just need to be distinct values
        index = -13;
        int max = -1;
    ...
    public Node visitSoar(SoarParser.SoarContext ctx) {
        ...
        boolean generateNoneBut = getGlobalDeclaration();

        Element attributeValuePairs = getScheduler(generateNoneBut);
    ...
    private String getProductionDeclaration(String productionName, Set<String> operatorIndexes, Map<String, Integer> conditionTempToIndex, Map<String, Integer> attributeTempToIndex) {
        ...
        if (_productionToOSupported.get(productionName)) {
            ...
            if (first) { //if there's at least one empty if() in declarations in uppaal tempale, the first if() will become if(false)
                currentTemplateDeclaration.append("false");
            }
            currentTemplateDeclaration.append(") {\n");
...
//        ('o-support' | 'i-support' | 'chunk' | 'default' | 'template' )
     ...
     * @param productionValues List of values   //(HK) Todo: probably better if the top three(id, attr, val) are packaged into one datastructure
     ...
     */
    private StringBuilder[][] getConditionOrAssignment(LinkedList<String> productionIdentifiers, LinkedList<String> productionAttributes,
                                                       LinkedList<String> productionValues, LinkedList<String> productionValueTemps,
                                                       LinkedList<String> productionAttributeTemps, Map<String, Integer> conditionTempToIndex,
                                                       Map<String, Integer> attributeTempToIndex, boolean isAssignment, Map<Integer,
                                                        StringBuilder> indexToKickBackCommands) {
    ...
    public Node visitSoar_production(SoarParser.Soar_productionContext ctx) {
        ...
        Node n = ctx.condition_side().accept(this);
        ...
        boolean needsRetraction = !_productionToOSupported.get(ctx.sym_constant().getText()); //checks if the production that is O-Sup needs retraction
     ...
     * @return true if the innerConditionVisit worked as expected false otherwise TODO: Throws away output-link currently
     */
    private boolean innerConditionVisit(List<SoarParser.Attr_value_testsContext> attrValueTestsCtxs, Map<String, String> localVariableDictionary, String idTest, ProductionVariables localActualVariables, Map<String, String> attributeVariableToArrayName) {
        // TODO: Throws away output-link currently
        for (SoarParser.Attr_value_testsContext attributeCtx : attrValueTestsCtxs) {
            for (int i = 0; i < attributeCtx.attr_test().size(); i++) {
                if (attributeCtx.attr_test(i).getText().equals("output-link")) {
                    return false;
                }
            }
        }
        ...
        if (localVariableDictionary.containsKey(idTest)) {
            ...
            for (SoarParser.Attr_value_testsContext attributeCtx : attrValueTestsCtxs) {
                ...
                for (int i = 0; i < attributeCtx.attr_test().size() - 1; i++) {
                    String attributeText = SoarTranslator.simplifiedString(attributeCtx.attr_test(i).getText());
                        ...
                        if (attributeCtx.value_test(0).test().conjunctive_test() != null || attributeCtx.value_test(0).test().simple_test().disjunction_test() == null) {
                            ...
                            if (relationAndRightTerm.getProperty("setOfRel") != null) {
                                ...
                                for (int i = 0; i < relations.length; i++) {
                                    ...
                                    if (_conditionSideVariablesToTemps.containsKey(variablesForRelations[i])) {
                                        variablesForRelations[i] = _conditionSideVariablesToTemps.get(variablesForRelations[i]);
                                    }
                                }
                            }
                           ...
                           if (value.length() > 0) {
                               ...
                               // todo: Optimize the order that the conditions are checked
                                if (value.equals("nilAnything")) {
                                    /**Here you were adding a the front of the list(i.e, from the right)**/
...
//                                    _latestIndex++;
                                    /**Recent change: Now you are adding from the end of the list(i.e, from the left)
                                     * You do the same thing below if condition for relations and extraRestriction**/
                                    _conditionProductionValues.add(value);
                                    _conditionProductionIdentifiers.add( _conditionProductionIdentifiers.removeLast());
                                    _conditionProductionAttributes.add(_conditionProductionAttributes.removeLast());
                                    _conditionProductionTemp.add(_conditionProductionTemp.removeLast());
                                    _attributeTemps.add( _attributeTemps.removeLast());
                                } else {
                                    _conditionProductionValues.add(value);
                                    _conditionProductionTemp.add("0");
                                }
...
//                                        _latestIndex++;
//                                    }
                                    String identifier = _conditionProductionTemp.get(_conditionProductionTemp.size()- 1); //_conditionProductionTemp.size() should get the latest value
                                    for(int i = 0; i < relations.length; i++) {
                                        /* "extraRestriction" sometimes variables have extraRestriction Eg.,
                                        <s> ^value {<v> > 5}
                                        <s> ^value <v> -> dummy_state_value_$_v_$     **now this dummy value can be given a extraRestriction
                                        dummy_state_value_$_v_$ extraRestriction 5 greaterThan //Here the "RELATION" is greaterThan
                                         */
                                        _conditionProductionIdentifiers.add(identifier);
                                        _conditionProductionAttributes.add("extraRestriction");
                                        _conditionProductionValues.add(variablesForRelations[i]);
                                        _conditionProductionTemp.add(relations[i]);
                                        _attributeTemps.add("0");
                                    }
                                }
                            }
    ...
    private String getVariableOrTemp(SoarParser.ValueContext ctx) {
        ...
        if (ctx.variable() == null) {
            ...
            variableOrTemp.append(_conditionSideVariablesToTemps.get(SoarTranslator.simplifiedString(ctx.variable().getText())));
        }
     ...
     * @param generateNoneBut Whether to generate none but switch or not
     * @return The AV template
     */
    private Element getScheduler(boolean generateNoneBut) {
        getRunScheduler();
        getAVUtility(generateNoneBut);
    ...
    private Element getRunScheduler() {
        ...
        //HK new Horizontal lines - starting lines in scheduler that maps dummyio 1,23 variables
        /**name: name of the state,
         * stackGuard:this is the guard(on the first edge it can be 'null', all other edges its "!addOperator",
         * assignment: update in uppaal
         * synchronization: Run_Rule!**/

        //first edge, for id:_state, attr: io, val: dummyio1
        String nextAssignment = "addOperator = true, \nvalueFunction = 0, \nglobalIdentifier = _state, \n globalAttribute = io, \nglobalValue = dummy_io1, \nglobalDoesContain = 1";
        lastLocation = addHorizontalLocation(runScheduler, lastLocation, lastLocationCoords, "Input", null, null, nextAssignment, false, null);

        //second edge, for id:dummyio1, attr: input_link, val: dummyio2
        nextAssignment = "addOperator = true,\nvalueFunction = 0, \nglobalIdentifier = dummy_io1,\nglobalAttribute = input_link,\nglobalValue = dummy_io2,\nglobalDoesContain = 1";
        lastLocation = addHorizontalLocation(runScheduler, lastLocation, lastLocationCoords, "Next_State", null, "!addOperator", nextAssignment, false, null);

        //third edge, for id:dummyio2, attr: flightData, val: dummyio3
        nextAssignment = "addOperator = true,\nvalueFunction = 0,\nglobalIdentifier = dummy_io2,\nglobalAttribute = flightdata,\nglobalValue = dummy_io3,\nglobalDoesContain = 1";
        lastLocation = addHorizontalLocation(runScheduler, lastLocation, lastLocationCoords, "Set_Superstate_Check", null, "!addOperator", nextAssignment, false, null);


        nextAssignment = "addOperator = true,\nvalueFunction = 0,\nglobalIdentifier = _state,\nglobalAttribute = superstate,\nglobalValue = nil,\nglobalDoesContain = 1";
        lastLocation = addHorizontalLocation(runScheduler, lastLocation, lastLocationCoords, "Set_Superstate_set", null, "!addOperator", nextAssignment, false, null);
        ...
        goBackToProvidedLocation(runScheduler, lastLocation, proposalLocation, lastLocationCoords, null, "halt", false); //HK: back edge from Run_retract to Get_Input
        ...
        //(HK)Keep in mind: for an edge, guard and update in uppaal, while coding guard:"...", assignment:" <for update>"
        //below line: making an edge from last location to 'proposal'--> this should put a edge in uppal in scheduler from ready_descion to 'Proposal' with below guard and update
    ...
    private void makeLoop(String direction, Template currentTemplate, Location loopLocation, String guard, String assignment) {
        ...
        Integer[] nails = new Integer[]{loopLocation.getX(),
                                        loopLocation.getY() + constants[1],
                                        loopLocation.getX() - constants[2],
                                        loopLocation.getY() + constants[1],
                                        loopLocation.getX() - constants[2],
                                        loopLocation.getY()};
        makeEdgeWithNails(currentTemplate, loopLocation, loopLocation, null, null, null,
                          new Integer[]{xTextLocation, loopLocation.getY() - 10 - (SIZE_OF_TEXT * (getNumLines(guard, " &&") + getNumLines(assignment, ",")))},
                guard, new Integer[]{xTextLocation, loopLocation.getY() - 10}, assignment,
                new Integer[]{xTextLocation, loopLocation.getY() - 10 + getNumLines(guard, " &&")*SIZE_OF_TEXT}, nails);
    }
     ...
     * @param generateNoneBut Whether to generate nonebut switch
     * @return The AVUtility template
     */
    private Element getAVUtility(boolean generateNoneBut) {
        ...
        AVUtilityTemplate.setProperty("declaration",
                ...
                "void catchAll() {\n" +
                 ...
                 "}\n" +
                  "\n");
        if (generateNoneBut) {
            AVUtilityTemplate.setProperty("declaration", getText(AVUtilityTemplate, "declaration") +
                "void nextNumBut() {\n" +
        ...
        for (Map.Entry<String, LinkedList<String>> nextVariableToAttribute : _variableToAttributes.entrySet()) {
            ...
            for (String nextAttribute : nextVariableToAttribute.getValue()) {
                if (count != 0) {
                    ...
                    catchAll.append("globalAttribute != io ||\n");
                    count++;
                }
            ...
            shift += count*2;
            catchAll.append(")").append(")");
        }
        if (catchAll.length() > 0) {
            catchAll.append(" &&\n");
        }

        catchAll.append("(globalIdentifier == dummy_io1 "+ "&&\nglobalAttribute != input_link) ||\n");
        catchAll.append("(globalIdentifier == dummy_io2 "+ "&&\nglobalAttribute != flightdata) ||\n");
        catchAll.append("(globalIdentifier == dummy_io3 "+ "&&\n(globalAttribute != takeOver ||\nglobalAttribute !=populated ||\n globalAttribute !=currentTime))\n");
